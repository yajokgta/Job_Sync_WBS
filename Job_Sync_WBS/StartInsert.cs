...
namespace Job_Sync_WBS
{
    internal class StartInsert
    {
        ...
        public static async void insertDB()
        {
            List<INFWBSElement> custommodel = new List<INFWBSElement>();
            SinghaDbDataContext db = new SinghaDbDataContext(constr);
            custommodel = CallAPI.getAllSAP();
            Console.WriteLine("DATA RECORD : "+custommodel.Count);
            /*db.ExecuteCommand("DELETE FROM dbo.INFWBSElement");
            using (var copy = new SqlBulkCopy(constr))
            {
